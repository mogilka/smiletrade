...
class I18nNavigationFrontend {
    ...
    if (!I18N_USE_CACHE || !file_exists($cachefile)) {
      ...
      foreach ($data->page as $pagedata) {
        ...
        foreach ($pagedata as $propdata) {
          self::$pages[$url][$propdata->getName()] = '' . $propdata;
        }
  ...
  public static function getPageStructure($slug=null, $menuOnly=true, $slugToIgnore=null) {
    $slug = '' . $slug;
    $structure = array();
    self::getPageStructureImpl($structure, $slug, $menuOnly, $slugToIgnore);
    return $structure;
  }
  
  private static function getPageStructureImpl(&$structure, $slug, $menuOnly=true, $slugToIgnore=null) {
    $pages = self::getPages();
    ...
    if (isset($pages[$slug]['children'])) {
      foreach ($pages[$slug]['children'] as $childslug) {
        if ($childslug != $slugToIgnore && (!$menuOnly || $pages[$childslug]['menuStatus'] == 'Y')) {
          ...
          self::getPageStructureImpl($structure, $childslug, $menuOnly, $slugToIgnore);
        }
      }
    }
  }
